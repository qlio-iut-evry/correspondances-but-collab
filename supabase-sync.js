...
async function selectProject(projectId) {
  ...
  subscribeToRealtime();
  renderProjectBadge();
  // R51/R57: piles d'annulation/rétablissement propres à ce projet (clés
  // localStorage scopées par CURRENT_PROJECT_ID) — volontairement pas dans
  // loadProjectState() elle-même, qui est aussi appelée à chaque changement
  // distant reçu en temps réel (onRemoteChange) et ne doit pas réinitialiser
  // l'undo/redo en cours.
  if (typeof restoreUndoStack === 'function') restoreUndoStack();
  if (typeof restoreRedoStack === 'function') restoreRedoStack();
}
...
function subscribeToRealtime() {
  ...
  _realtimeChannel = db
    ...
    // R58: présence collaborative — jusqu'ici rien n'indiquait qu'un
    // collègue travaillait sur le même projet en ce moment, donc deux
    // personnes pouvaient modifier la même ressource simultanément sans le
    // savoir (seul recours après coup : la notification de changement
    // distant). Presence de Supabase Realtime est fait pour ça : état
    // éphémère par canal, propagé aux autres clients, retiré automatiquement
    // à la déconnexion — pas besoin de table ni de mécanisme de heartbeat
    // maison.
    .on('presence', { event: 'sync' }, renderPresence)
    .subscribe(async status => {
      updateSyncIndicator(status === 'SUBSCRIBED' ? 'online' : 'connecting');
      if (status === 'SUBSCRIBED' && CURRENT_USER) {
...
/** Affiche qui d'autre travaille actuellement sur ce projet (présence Realtime). */
function renderPresence(){
  const el = document.getElementById('presence-indicator');
  if (!el || !_realtimeChannel) return;
  const state = _realtimeChannel.presenceState();
  // Un même utilisateur peut apparaître sous plusieurs clés s'il a plusieurs
  // onglets ouverts — dédoublonné par user_id pour ne compter chaque
  // personne qu'une fois.
  const seen = new Map();
  Object.values(state).flat().forEach(p => { if (p.user_id) seen.set(p.user_id, p); });
  const others = [...seen.values()].filter(p => p.user_id !== CURRENT_USER?.id);
  if (!others.length) { el.style.display = 'none'; return; }
  const names = others.map(p => p.display_name || '?');
  el.style.display = '';
  el.textContent = `👥 ${others.length===1 ? '1 collègue' : others.length+' collègues'} en ligne`;
  el.title = 'Également sur ce projet : ' + names.join(', ');
}
...
function renderUserBadge(user) {
  ...
  ui.innerHTML = `
    ...
    <span id="presence-indicator" style="display:none;font-size:11px;color:rgba(255,255,255,.9);white-space:nowrap;background:rgba(255,255,255,.12);padding:3px 9px;border-radius:12px;cursor:default"></span>

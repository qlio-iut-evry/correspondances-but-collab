...
-- ── Ressources "extras" (partage exceptionnel secondaire) ─────
-- Quand plusieurs ressources anciennes sont cochées à la fois dans la modale
-- d'association, la première devient l'affectation principale (table
-- overrides) et les suivantes sont des "extras" : une ancienne ressource
-- supplémentaire rattachée à la même nouvelle ressource. Une ligne par
-- couple (new_code, old_code) — plusieurs lignes possibles pour un même
-- new_code. Sans cette table, ces affectations secondaires ne vivaient que
-- dans l'onglet du navigateur en cours et disparaissaient au rechargement.

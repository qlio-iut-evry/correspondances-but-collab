...
-- NE PAS exécuter isolément sur le projet Supabase existant : ce contenu
-- est déjà en place (appliqué à la main avant l'existence de ce dossier).
-- Conservé ici comme référence historique et pour qu'un nouveau projet qui
-- préférerait partir des migrations plutôt que de sql/schema.sql retrouve
-- l'état exact migration par migration. Pour un nouveau projet Supabase,
-- utiliser sql/schema.sql (qui inclut déjà tout ceci) — voir README.md.
